...
export default function ProductList({ products }: { products: Product[] }) {
  ...
  return (
    <>
    <div className="newcontainer" role="list">
      {products.slice(0, visibleCount).map((pro, index) => (
        <div key={pro.id}  className="productbox" role="listitem">
            <Image
            src={pro.images[0]}
            alt={`Image of ${pro.title}`}// screen readers
            role="img"
            width={200}
            height={200}
      ...
      <div style={{textAlign:'center'}}>
            ...
            aria-disabled={visibleCount >= products.length} // screen readers
            style={{
                padding: "10px 20px",
